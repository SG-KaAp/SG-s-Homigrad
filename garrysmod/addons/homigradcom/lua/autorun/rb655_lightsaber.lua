...
rb655_AddForcePower( {
	...
	action = function( self, ply )
		...
		-- Если ply.prijok уже true, просто обновляем таймер
		if ply.prijok == true then
			if timer.Exists( "ForceLeapPrijok_" .. ply:EntIndex() ) then
				timer.Adjust( "ForceLeapPrijok_" .. ply:EntIndex(), 3, 1, function()
					ply.prijok = false
				end )
			end
		else
			-- Устанавливаем ply.prijok на true
			ply.prijok = true

			-- Создаем или обновляем таймер
			timer.Create( "ForceLeapPrijok_" .. ply:EntIndex(), 3, 1, function()
				ply.prijok = false
			end )
		end
	end
