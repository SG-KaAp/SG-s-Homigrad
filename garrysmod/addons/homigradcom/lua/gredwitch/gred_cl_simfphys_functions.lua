...
gred.InitializeSimfphys = function(vehicle,callback)
	...
	timer.Create("GRED_TANK_INIT_"..vehicle.EntityIndex,1,1,function()
		...
		vehicle.Think = function(vehicle)
			...
			gred.TankHandleSeats(vehicle,Mode,LocalPly,VehicleSeatTab,ViewPortsTab,VehicleTab)
...
gred.TankInitHookHacks = function(vehicle)
	if !gred.SimfphysCalcViewHack then
		local HookTab = hook.GetTable()
		
		if HookTab.CalcView and HookTab.CalcView.zz_simfphys_gunner_view then
...
gred.TankHandleSeats = function(vehicle,Mode,ply,VehicleSeatTab,ViewPortsTab,VehicleTab)
	if vehicle.LocalPlayerActiveSeat then
		...
		if SeatTab then
			if (vehicle.LocalPlayerActiveSeat.MuzzleAttachment or SeatTab.IsLoader or SeatTab.Hatches or SeatTab.IsCommander or SeatTab.HasRadio) and !ply.HookAdded then
				...
				local HUDPaint = VehicleTab.HUDPaint
				...
				if HUDPaint then
					hook.Add("HUDPaint","gred_simfphys_tank_HUDPaint",function()
						...
						HUDPaint(vehicle,vehicle.LocalPlayerActiveSeat,vehicle.LocalPlayerActiveSeatID,SeatTab,Mode,ply,ScrW,ScrH,LoadingSeatTab,LoadingSeat)
					end)
				else
					hook.Add("HUDPaint","gred_simfphys_tank_HUDPaint",function()
						if !IsValid(vehicle) then return end
	...
	if (SeatTab.RequiresHatch and SeatTab.RequiresHatch[seat:GetNWInt("HatchID",0)] or (!SeatTab.RequiresHatch and seat:GetNWInt("HatchID",0) == 0)) then
		if SeatTab.IsLoader and LoadingSeat != seat then
			if IsValid(LoadingSeat) and LoadingSeatTab and LoadingSeatTab.Primary then
				...
				if LoadingPrimaryTab then
					...
					draw.SimpleText(LoadingPrimaryTab.ExactName or (LoadingPrimaryTab.Type == "Cannon" and LoadingPrimaryTab.ShellTypes[1].Caliber.."MM " 
					...
					if LoadingPrimaryTab.Type == "Cannon" then
						...
						draw.SimpleText(LoadingPrimaryTab.ShellTypes[ShellType].ExactShellType or LoadingPrimaryTab.ShellTypes[ShellType].ShellType,"SIMFPHYS_ARMED_HUDFONT",X,HeightCalc,SIMFPHYS_COLOR,0,1)
						Height = Height + HeightAdd
						HeightCalc = ScrH*Height
		...
		if SeatTab.Primary or SeatTab.Secondary then
			if SeatTab.Primary  then
				...
				if PrimaryTab then
					draw.SimpleText("PRIMARY","SIMFPHYS_ARMED_HUDFONT",7,HeightCalc,SIMFPHYS_COLOR,0,1)
					draw.SimpleText(PrimaryTab.ExactName or (PrimaryTab.Type == "Cannon" and PrimaryTab.ShellTypes[1].Caliber.."MM " 
					...
					if PrimaryTab.Type == "Cannon" then
						...
						draw.SimpleText("SHELLTYPE","SIMFPHYS_ARMED_HUDFONT",7,HeightCalc,SIMFPHYS_COLOR,0,1)
						draw.SimpleText(PrimaryTab.ShellTypes[ShellType].ExactShellType or PrimaryTab.ShellTypes[ShellType].ShellType,"SIMFPHYS_ARMED_HUDFONT",X,HeightCalc,SIMFPHYS_COLOR,0,1)
						Height = Height + HeightAdd
						HeightCalc = ScrH*Height
					end
				...
				if SecondaryTab then
					...
					draw.SimpleText(SecondaryTab.ExactName or (SecondaryTab.Type == "Cannon" and SecondaryTab.ShellTypes[1].Caliber.."MM " 
