...
gred.TankToggleHatch = function(seat,SeatID,vehicle,ply,HatchID,OldHatch,SeatTab)
	if HatchID == 0 then
		...
		if SeatID == 0 then
			...
			SetPos(seat,EntityWorldToLocal(vehicle,GetPos(vehicle) + GetUp(seat) * (-34 + vehicle.SeatOffset.z) + GetRight(seat) * (vehicle.SeatOffset.y) + GetForward(seat) * (-6 + vehicle.SeatOffset.x)) + HatchTab.PosOffset)
		else
			if SeatTab.Attachment then
				...
				local Pos,Ang = LocalToWorld(vehicle.PassengerSeats[SeatID].pos - att.LPos + HatchTab.PosOffset,vehicle.PassengerSeats[SeatID].ang - att.LAng,att.LPos,att.LAng)
				
				SetPos(seat,EntityLocalToWorld(vehicle, Pos))
				
				SetAngles(seat,LocalToWorldAngles(vehicle,Ang + HatchTab.AngOffset))
			else
				SetPos(seat,EntityLocalToWorld(vehicle, vehicle.PassengerSeats[SeatID].pos + HatchTab.PosOffset))
...
gred.TankResetHatch = function(vehicle,seat,SeatID,vehicle,SeatTab)
	...
	if SeatID == 0 then
		...
		if SeatTab.Attachment then
			...
			if att then
				att.LPos,att.LAng = EntityWorldToLocal(vehicle,att.Pos),WorldToLocalAngles(vehicle,att.Ang)
				local Pos,Ang = LocalToWorld(vehicle.PassengerSeats[SeatID].pos - att.LPos,vehicle.PassengerSeats[SeatID].ang - att.LAng,att.Pos,att.LAng)
				
				SetPos(seat,Pos)
				SetAngles(seat,LocalToWorldAngles(vehicle,Ang))
			end
		else
			local PosH,AngH
			...
			SetPos(seat,PosH or EntityLocalToWorld(vehicle, vehicle.PassengerSeats[SeatID].pos))
