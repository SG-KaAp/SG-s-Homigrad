...
function TOOL:SetupObjectPanel( panel )
    ...
    numTimeBeforeHurt:SetText( "#mappatcher.tools.hurt.settings.time_before_hurt" )
    ...
    numHurtInterval:SetText( "#mappatcher.tools.hurt.settings.interval" )
    ...
    numHurtAmount:SetText( "#mappatcher.tools.hurt.settings.amount" )
    ...
    lblDamageType:SetText( "#mappatcher.tools.hurt.settings.damage_type" )

    local cmbDamageType = vgui.Create( "DComboBox", panel )
    cmbDamageType:SetPos( 137, 85 )
    cmbDamageType:SetSize( 123, 20 )
    cmbDamageType:AddChoice( "#mappatcher.tools.hurt.settings.damage_generic", DMG_GENERIC, self.damage_type == DMG_GENERIC )
    cmbDamageType:AddChoice( "#mappatcher.tools.hurt.settings.damage_poison", DMG_POISON, self.damage_type == DMG_POISON )
    cmbDamageType:AddChoice( "#mappatcher.tools.hurt.settings.damage_dissolve", DMG_DISSOLVE, self.damage_type == DMG_DISSOLVE )
