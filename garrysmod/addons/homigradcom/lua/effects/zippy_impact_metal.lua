...
function EFFECT:Init(data)
    ...
    flash:SetEndAlpha(0)
    flash:SetColor(255,200,155)
    flash:SetEndSize(0)
    flash:SetDieTime(0.075)

    flash:SetStartSize(math.random(66, 75)*intensity)

    if GetConVar("zippyimpacts_dynlight_intensity"):GetFloat() <= 0 then return end
	...
	if dlight then
		...
		dlight.g = 125
		dlight.b = 0
		dlight.brightness = math.Rand(0, 1)*GetConVar("zippyimpacts_dynlight_intensity"):GetFloat()
		dlight.Decay = 1000
		dlight.Size = 256*intensity*GetConVar("zippyimpacts_dynlight_intensity"):GetFloat()
