if engine.ActiveGamemode() == "homigradcom" then
	...
	CreateClientConVar("hg_armercam","0",true,false,nil,0,2)
	...
	["weapon_slb_sg552"] = true,
	["weapon_slb_g3sg1"] = true,
	["weapon_slb_sg550"] = true,
	["weapon_slb_aug"] = true,

	["weapon_slb_ak74"] = true,
	...
	function CalcView(ply,vec,ang,fov,znear,zfar)
		...
		if GetConVar("hg_bodycam"):GetInt() == 0 then
			...
			vecEye = (eye and bodypos + bodyang:Up() * 0 + bodyang:Forward() * 16 + bodyang:Right() * -6) or lply:EyePos()
		end
		...
		if ply:Alive() and ply:GetNWBool("fake") and IsValid(ragdoll) then
			...
			if GetConVar("hg_bodycam"):GetInt() == 1 then
				...
				eyeAngs = att.Ang
				att.Pos = (eye and bodypos + bodyang:Up() * -2 + bodyang:Forward() * 16 + bodyang:Right() * 2) or lply:EyePos()
			end
	...
	if wep and weps[wep:GetClass()] then
		...
		if weaponClass == "weapon_slb_ak74" then
			--Vector(3.85,10,1.45)
			vecWep = hand.Pos + hand.Ang:Up() * 5 - hand.Ang:Forward() * 10 + hand.Ang:Right() * 0.7
			angWep = hand.Ang + Angle(-5,10,0)
		end
		...
		if weaponClass == "weapon_slb_awp" then
			--Vector(6.2,6,0.88)
			vecWep = hand.Pos + hand.Ang:Up() * 6.15 - hand.Ang:Forward() * 6 + hand.Ang:Right() * 0.88
			angWep = hand.Ang + Angle(-7,0,0)
		end
		if weaponClass == "weapon_slb_scout" then
			--Vector(6.2,6,0.88)
			vecWep = hand.Pos + hand.Ang:Up() * 5.15 - hand.Ang:Forward() * 6 + hand.Ang:Right() * 0.88
			angWep = hand.Ang + Angle(-7,0,0)
		end
		if weaponClass == "weapon_slb_aug" then
			--Vector(6.2,6,0.88)
			vecWep = hand.Pos + hand.Ang:Up() * 5.15 - hand.Ang:Forward() * 8 + hand.Ang:Right() * 0.7
			angWep = hand.Ang + Angle(-7,0,0)
		end
		if weaponClass == "weapon_slb_g3sg1" then
			--Vector(6.2,6,0.88)
			vecWep = hand.Pos + hand.Ang:Up() * 6.15 - hand.Ang:Forward() * 9 + hand.Ang:Right() * 0.88
			angWep = hand.Ang + Angle(-7,0,0)
		end
		if weaponClass == "weapon_slb_sg550" then
			--Vector(6.2,6,0.88)
			vecWep = hand.Pos + hand.Ang:Up() * 4.55 - hand.Ang:Forward() * 6 + hand.Ang:Right() * 0.88
			angWep = hand.Ang + Angle(-7,0,0)
		end
		if weaponClass == "weapon_slb_sg552" then
			--Vector(6.2,6,0.88)
			vecWep = hand.Pos + hand.Ang:Up() * 5.35 - hand.Ang:Forward() * 6 + hand.Ang:Right() * 0.88
			angWep = hand.Ang + Angle(-7,0,0)
		end
	end
	...
	local hg_armercam = GetConVar("hg_armercam"):GetInt()
	
	
		if lply:InVehicle() or not firstPerson or hg_armercam == 2 then return end
	
		if not lply:Alive() or (IsValid(wep) and whitelistweps[wep:GetClass()]) or lply:GetMoveType() == MOVETYPE_NOCLIP or hg_armercam == 1 then
