...
function alkoriot.StartRoundSV()
	tdm.RemoveItems()
...
function alkoriot.RoundEndCheck()
...
function alkoriot.EndRound(winner) tdm.EndRoundMessage(winner) end

function alkoriot.PlayerInitialSpawn(ply) ply:SetTeam(math.random(1,2)) end

function alkoriot.PlayerSpawn(ply,teamID)
	local teamTbl = alkoriot[alkoriot.teamEncoder[teamID]]
	...
	if teamID == 1 then
		JMod.EZ_Equip_Armor(ply,"alkoriot-Helmet",Color(65,65,65))
...
function alkoriot.PlayerCanJoinTeam(ply,teamID)
...
function alkoriot.ShouldSpawnLoot()
...
function alkoriot.PlayerDeath(ply,inf,att) return false end
