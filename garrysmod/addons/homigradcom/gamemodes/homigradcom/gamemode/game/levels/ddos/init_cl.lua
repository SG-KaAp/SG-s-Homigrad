/*ddos.GetTeamName = tdm.GetTeamName

local playsound = false

function ddos.StartRoundCL()
    playsound = true
end

function ddos.HUDPaint_RoundLeft(white)
    local lply = LocalPlayer()
	local name,color = ddos.GetTeamName(lply)
...
hook.Add("player_activate","ddos_SendData",function(data)
    ddos.points = {}

    ddos.WinPoints = {}

    for i = 1, 2 do
        ddos.WinPoints[i] = GetGlobalInt("ddos_Winpoints" .. i)
    end

    timer.Create("ddos_ThinkAboutPoints", 1, 0, function() --подумай о точках... засунул в таймер для оптимизации, ибо там каждый тик игроки в сфере подглядываются, ну и в целом для удобства
        ddos.PointsThink()
    end)

    for k, v in pairs(ddos.points) do
...
function ddos.PointsThink()
    for i = 1, 3 do
        ddos.WinPoints[i] = GetGlobalInt("ddos_Winpoints" .. i, 0)
    end
...
function ddos.HUDPaint_RoundLeft(white2) --позиции точек и счёт
    local lply = LocalPlayer()
	local name,color = ddos.GetTeamName(lply)
    ...
    surface.DrawRect(ScrW() * 0.39 + ((1000 - ddos.WinPoints[1]) / 1000 * ScrW() * 0.1), ScrH() * 0.97, ddos.WinPoints[1] / 1000 * ScrW() * 0.1, ScrH() * 0.01)
    --print(ddos.WinPoints[1])
    --print("gega")
    --print(ddos.WinPoints[2])
    surface.SetDrawColor(blue)
    surface.DrawRect(ScrW() * 0.51, ScrH() * 0.97, ddos.WinPoints[2] / 1000 * ScrW() * 0.1, ScrH() * 0.01)
    ...
    local respawntime = GetGlobalInt("ddos_respawntime", CurTime())
