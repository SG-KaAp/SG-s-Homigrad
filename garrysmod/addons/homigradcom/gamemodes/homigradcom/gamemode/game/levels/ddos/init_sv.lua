...
function ddos.SpawnCommand(tbl,aviable,func,funcShould)
...
function ddos.DirectOtherTeam(start,min,max)
...
function ddos.GetListMul(list,mul,func,max)
	local newList = {}
...
/*function ddos.StartRoundSV()
	...
	local spawnsT,spawnsCT = ddos.SpawnsTwoCommand()
	ddos.SpawnCommand(team.GetPlayers(1),spawnsT)
	ddos.SpawnCommand(team.GetPlayers(2),spawnsCT)
end*/
...
function ddos.Think()
    ddos.LastWave = ddos.LastWave or CurTime() + 60

    if CurTime() >= ddos.LastWave then
        SetGlobalInt("ddos_respawntime", CurTime())
        ...
        for ent in pairs(ddos.ragdolls) do
            if IsValid(ent) then ent:Remove() end
            ddos.ragdolls[ent] = nil
        end
        ddos.LastWave = CurTime() + 60
    end
end

function ddos.GetCountLive(list,func)
	local count = 0
	local result
...
function ddos.PointsThink()
    local ddos_points = ddos.points
    for i, point in pairs(SpawnPointsList.controlpoint[3]) do
        local v = ddos_points[i]
        if not v then
            v = {}
            ddos_points[i] = v
        end
        ...
        if v.CaptureTeam and v.CaptureTeam != 0 then
            ddos.WinPoints[v.CaptureTeam] = ddos.WinPoints[v.CaptureTeam] + 7.5 / #SpawnPointsList.controlpoint[3]
        end
    ...
    for i = 1, 2 do
        SetGlobalInt("ddos_Winpoints" .. i, ddos.WinPoints[i])
    end
end

function ddos.RoundEndCheck()
    tdm.Center()

    for i = 1, 2 do
        if ddos.WinPoints[i] >= 1000 then
            EndRound(i)
        end
    end
...
function ddos.EndRound(winner)
    ...
    timer.Remove("ddos_NewWave")
    timer.Remove("ddos_ThinkAboutPoints")
end

/*function ddos.RoundEndCheck()
	if roundTimeStart + roundTime - CurTime() <= 0 then EndRound() end

	local TAlive = ddos.GetCountLive(team.GetPlayers(1))
	local CTAlive = ddos.GetCountLive(team.GetPlayers(2))
...
function ddos.EndRoundMessage(winner,textNobody)
	local tbl = TableRound()
	PrintMessage(3,"Выиграли - " .. ((winner == 1 and tbl.red[1]) or (winner == 2 and tbl.blue[1]) or (textNobody or "Дружба")) .. ".")
end

function ddos.EndRound(winner) ddos.EndRoundMessage(winner) end*/

--

function ddos.GiveSwep(ply,list,mulClip1)
...
function ddos.PlayerSpawn(ply,teamID)
	local teamTbl = ddos[ddos.teamEncoder[teamID]]
	...
	ddos.GiveSwep(ply,teamTbl.main_weapon)
	ddos.GiveSwep(ply,teamTbl.secondary_weapon)
...
function ddos.PlayerInitialSpawn(ply) ply:SetTeam(math.random(2)) end

function ddos.PlayerCanJoinTeam(ply,teamID)
    if teamID == 3 then ply:ChatPrint("Иди нахуй") return false end
end

function ddos.PlayerDeath(ply,inf,att)
    ddos.ragdolls[ply:GetNWEntity("Ragdoll")] = true
    return false
end

--function ddos.PlayerDeath(ply,inf,att) return false end