...
function lizardsmustdie.StartRoundSV()
	tdm.RemoveItems()
...
function lizardsmustdie.RoundEndCheck()
...
function lizardsmustdie.EndRound(winner) tdm.EndRoundMessage(winner) end

function lizardsmustdie.PlayerInitialSpawn(ply) ply:SetTeam(math.random(1,2)) end

function lizardsmustdie.PlayerSpawn(ply,teamID)
	local teamTbl = lizardsmustdie[lizardsmustdie.teamEncoder[teamID]]
	...
	if teamID == 1 then
		JMod.EZ_Equip_Armor(ply,"lizardsmustdie-Helmet",Color(65,65,65))
...
function lizardsmustdie.PlayerCanJoinTeam(ply,teamID)
...
function lizardsmustdie.ShouldSpawnLoot()
...
function lizardsmustdie.PlayerDeath(ply,inf,att) return false end
