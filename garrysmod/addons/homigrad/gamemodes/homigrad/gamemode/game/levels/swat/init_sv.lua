function swat.StartRoundSV(data)
    tdm.RemoveItems()
...
function swat.RoundEndCheck()
    if roundTimeStart + roundTime < CurTime() then
		if not swat.police then
			swat.police = true
			PrintMessage(2,"Спецназ приехал.")
...
function swat.PlayerInitialSpawn(ply) ply:SetTeam(2) end
...
function swat.GuiltLogic(ply,att,dmgInfo)
	if att.isContr and ply:Team() == 2 then return dmgInfo:GetDamage() * 3 end
end

function swat.NoSelectRandom()
